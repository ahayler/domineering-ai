...
public class MinMaxAI_V4 extends AI {
    /*
    V4 implements bias functions
    */
    ...
    public MinMaxAI_V4(int depth, EvaluationFunction evaluationFunction, boolean useSafeMovePruning,
    ...
    public Coordinate getMove(char[][] board, Player player, int depth) {
        ...
        // clear the hashtable so we don't have depth problems
        hashtable.clear();
    ...
    public Coordinate minMaxHead(Tile[][] tileBoard, int depth, Player player) {
        ...
        if (player == Player.V) {
            ...
            for (int i = 0; i < movesArray.length; i++) {
                ...
                if (evaluation[0] >= 1000) {
                    ...
                    return movesArray[i];
                        ...
                        (evaluation[0] * 10 + biasEvaluation[0] * 10 == maxFirst * 10 + biasFirst
                                && evaluation[1] + biasEvaluation[1] > maxSecond + biasSecond)) {
                    // is new max
                    maxFirst = evaluation[0];
                    maxSecond = evaluation[1];
                    biasFirst = biasEvaluation[0];
                    biasSecond = biasEvaluation[1];
            ...
            for (int i = 0; i < movesArray.length; i++) {
                ...
                } else if (evaluation[0] * 10 + biasEvaluation[0]  < minFirst * 10 + biasFirst ||
                        (evaluation[0] * 10 + biasEvaluation[0] == minFirst * 10 + biasFirst &&
                                evaluation[1] + biasEvaluation[1] < minSecond + biasSecond)) {
                    // is new max
